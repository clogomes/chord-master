...
### Fase 37 — Sistema de Revisão Espaçada (SuperMemo SM-2 & Leitner 5-Caixas)
- **Motor de Repetição Espaçada (`core/review_scheduler.py`)**: Algoritmo SuperMemo SM-2 completo com fator de facilidade (*ease*), escalamento de intervalos (1d → 6d → n×ease), contagem de lapsos e mapeamento para 5 Caixas de Leitner (Novo, Aprendizagem, Consolidação, Retido, Dominado).
- **Persistência Segura em `core/user_manager.py`**: Novo campo `spaced_review_data` no `UserProfile` com migração por `schema_version=2` e `.get()` com defaults que garantem retrocompatibilidade com perfis existentes. Novos métodos `record_atomic_review()` e `get_daily_review_queue()`.
- **Ecrã de Revisão Diária (`gui/screens/daily_review_screen.py`)**: Sessão completa de revisão com até 15 itens atômicos, visualização de Caixas Leitner, botões de auto-avaliação (❌ Errei / 🟡 Difícil / 🟢 Bom / 🌟 Fácil), reprodução de áudio para intervalos, opções MCQ para teoria/glossário e cartão de conclusão com XP ganho (+15 XP por item dominado).
- **Ligação aos Ecrãs Existentes**: `practice_ear.py`, `practice_staff.py` e `theory_quiz_widget.py` registam agora cada resposta no sistema de repetição espaçada com `skill_id` atómico (`interval:M3:asc`, `staff:treble:C4`, `theory:chap1:q0`, etc.).
- **Navegação e Menu Principal**: "🔄 Revisão de Hoje" adicionado à barra lateral e ao grelha do menu principal com acento violeta.
- **Testes Unitários (`tests/test_review_scheduler.py`)**: 26 novos testes que cobrem SM-2 (ease, intervalos, lapsos, mínimos), serialização round-trip, retrocompatibilidade de schema, geração de fila e integração com `UserManager`. Total: 220/220 testes.

### Fase 38 — Campo Harmónico Menor & Cadências
- **Novos Capítulos Teóricos (`core/theory_content.py`)**:
  - **Capítulo 17 — Campo Harmónico Menor & Tétrades Menores**: As 3 formas da escala menor (Natural/Eólio, Harmónica e Melódica), graus e funções tonais em Lá menor, tétrades (Im7, IIø7, IIIMaj7+, IVm7, V7, VIMaj7, VII°7), progressões fundamentais (`i–VI–III–VII`, `i–iv–V7–i`, descida cromática e `iiø–V7–i`) e guias práticos com dedilhados de piano e posições CAGED de viola.
