...
def generate_student_report_markdown(user: UserProfile, lang: str = "pt") -> str:
    """Generates a structured, beautiful Markdown student report card."""
    now_str = datetime.datetime.now().strftime("%d/%m/%Y às %H:%M" if lang == "pt" else "%m/%d/%Y at %H:%M")
    lvl = user.level_info
    ...
    if lang == "pt":
        ...
        md.append(f"# 🎓 ChordMaster — Progress Report & Study Certificate")
        md.append(f"**Student:** {user.avatar} {user.username}  |  **Issue Date:** {now_str}\n")
        md.append("---")
        md.append(f"## 🏆 Musical Mastery Level")
        
        # Need to translate level title here if possible, but level_info might be in PT. 
        # For simplicity, we keep the original title as it comes from level_info
        md.append(f"- **Current Level:** Level {lvl['level']} — {lvl['icon']} **{lvl['title']}**")
        md.append(f"- **Total Experience (XP):** `{user.xp} XP`")
        if lvl['xp_needed'] > 0:
            md.append(f"- **Next Level:** `{lvl['xp_needed']} XP` remaining for Level {lvl['level'] + 1} ({lvl['progress_pct']:.0f}% complete)")
        else:
            md.append(f"- **Status:** 🌟 Maximum Mastery Level Reached!")
        md.append("")
        md.append(f"## 📖 Theory Lessons Progress ({len(user.completed_lessons)}/{len(LESSON_IDS)} Completed)")
        for lid, ltitle in LESSON_IDS:
            status = "✅ Completed" if lid in user.completed_lessons else "⏳ Pending"
            # Title here is also PT from LESSON_IDS, ideally translated but not strictly required
            md.append(f"- {status} — **{ltitle}**")
        md.append("")
        md.append(f"## 📊 Training Category Performance")
        md.append(f"| Category | Attempts | Hits | Accuracy | Best Streak |")
        md.append(f"| :--- | :---: | :---: | :---: | :---: |")

    from core.categories import CATEGORY_NAMES_PT, CATEGORY_NAMES_EN
    
    for cat_key, st in user.categories.items():
        if lang == "pt":
            cat_name = CATEGORY_NAMES_PT.get(cat_key, cat_key.replace("_", " ").title())
        else:
            cat_name = CATEGORY_NAMES_EN.get(cat_key, cat_key.replace("_", " ").title())
    ...
    if lang == "pt":
        ...
        md.append(f"\n**Global Accuracy:** `{user.accuracy_rate:.1f}%` ({user.total_correct} hits in {user.total_attempts} exercises)\n")
        md.append(f"## 🏅 Badges and Achievements ({len(user.unlocked_achievements)}/{len(ACHIEVEMENT_LIBRARY)})")
        ...
        for ach_id in user.unlocked_achievements:
            ...
            if ach:
                if lang == "pt":
                    ...
                    md.append(f"- {ach.icon} **{ach.get_name(lang)}** (+{ach.xp_reward} XP) — *{ach.get_description(lang)}*")
    else:
        if lang == "pt":
            md.append("*Nenhuma conquista desbloqueada ainda. Continua a praticar lições e músicas!*")
        else:
            md.append("*No achievements unlocked yet. Keep practicing lessons and songs!*")
    md.append("")
    ...
    if lang == "pt":
        ...
        md.append("*Automatically issued by the ChordMaster App — Music Theory & Practice.*")

    return "\n".join(md)


def export_student_report_file(user: UserProfile, export_dir: Optional[str] = None, lang: str = "pt") -> str:
    ...
    if lang == "pt":
        ...
        filename = f"progress_report_{clean_name}.md"
    filepath = os.path.join(export_dir, filename)

    content = generate_student_report_markdown(user, lang=lang)
